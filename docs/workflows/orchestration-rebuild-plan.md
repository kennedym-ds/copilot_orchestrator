...
Reimagine the Copilot configuration around a conductor-led, multi-agent system that mirrors the GitHub Copilot Orchestra pattern and the latest VS Code agent platform. The goal is to orchestrate planning, implementation, review, and documentation flows with explicit handoffs, high-fidelity context management, and model-aware cost controls.

This initiative consolidates instructions, prompts, and chat modes into an extensible architecture that maximizes productivity while preserving compliance, quality, and security guardrails.
...
- **Model Utilization Strategy:** Align personas with high-reasoning models (Claude Opus 4.6, Codex 5.2, Claude Sonnet 4.5, Gemini 3 Pro) for planning/analysis and cost-efficient models (Claude Haiku 4.5, Gemini 3 Flash, Codex 5.2) for execution.
- **Async Custom Agent Execution:** Leverage `#runCustomAgent` to invoke planning, research, implementation, and review agents in parallel, reducing wall-clock time for complex work.
...
- Specialized planning, implementation, and review custom agents operate autonomously with strict TDD and structured outputs.
...
- Handoffs defined in custom agent frontmatter provide guided workflow progression with optional auto-submission of follow-up prompts.
- Context-isolated custom agents prevent context bloat by running in separate windows and returning only final results.
...
| --- | --- | --- |
| Chat modes (`.github/chatmodes/*.chatmode.md`) | Legacy personas in prior repo lack a unified conductor. | Consolidate into flow controllers, execution custom agents, and utility agents with clear scopes. |
...
- **Responsibilities:**
  - Invoke specialized custom agents via `#runCustomAgent` with bounded prompts and toolsets.
...
- **Model:** Default to Claude Sonnet 4.5 or Claude Opus 4.6 for reasoning; allow overrides via front matter.
- **Tools:** `runCustomAgent`, `todos`, `fetch`, `search`, `githubRepo`, `edit` (restricted), `changes`, `runCommands` when necessary.

## Specialized Custom Agents

| Persona | Primary Models | Scope | Tooling | Notes |
| --- | --- | --- | --- | --- |
| Planning Researcher | Claude Opus 4.6, Gemini 3 Pro, Claude Sonnet 4.5 | Deep code/document research, summarization, option analysis | `search`, `fetch`, `githubRepo`, `readFile`, `usages`, `problems` | Must stop at findings; no plans or edits. |
...
| Reviewer | Claude Sonnet 4.5, Claude Opus 4.6 | Reviews diffs, ensures tests, flags issues with severity tags | `changes`, `search`, `usages`, `problems` | Returns structured review (`APPROVED/NEEDS_REVISION/FAILED`). |
...
- Configure custom agent front matter `handoffs` so that Planner offers “Kick off Implementation”, Implementer offers “Send to Review”, Reviewer offers “Prepare Commit Summary”.
...
- For parallelizable sub-work, instruct Conductor to spawn multiple `#runCustomAgent` calls and aggregate results before proceeding.
...
3. **Instruction Files Alignment:**
   - Map existing `instructions/global/*.instructions.md` to the new conductor/custom agent expectations.
...
- Categorize prompts into planning, implementation, review, research, and support libraries, each referencing the corresponding custom agent mode via `mode` front matter.
...
- Publish diagrams illustrating the conductor/custom agent workflow (Mermaid or textual description).
...
| 1 | Conductor Skeleton | `conductor.agent.md`, plan/phase templates, updated planner custom agents | Prompt Guild | Week 2 |
| 2 | Execution Agents | `implementer.agent.md`, `reviewer.agent.md`, update TDD modes, add handoffs | Dev Experience | Week 3 |
...
- **Parallel Agent Conflicts:** Conductor aggregates custom agent outputs sequentially and reconciles conflicting edits before applying patches.
...
- Reduction in average implementation cycle time by ≥25% due to custom agent parallelism.
...
4. Refactor planning and TDD chat modes into new custom agent taxonomy.
