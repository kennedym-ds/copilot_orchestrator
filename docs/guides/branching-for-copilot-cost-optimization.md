...
GitHub Copilot's premium request system charges differently per model — from 3× for Claude Sonnet 4.6 down to 0× for GPT-5 mini. If you're running a multi-agent system with 29 specialized agents, those multipliers add up fast.

Here's how we solved it: **three branches, automatically synced, each targeting a different cost profile.**

## The Problem

Not every task needs the most expensive model. A lint check doesn't need Claude Sonnet 4.6. A rubber duck debugging session doesn't need Claude Haiku 4.5. But maintaining separate agent configurations per cost tier by hand is a maintenance nightmare — every change to an agent's prompt, workflow, or tooling would need to be applied three times.
...
|------|-------|------|--------|-----|
| Premium | Claude Sonnet 4.6 | 3× | 3 (conductor, planner, security) | Orchestration errors compound. Security is ruin-risk. Plan quality drives everything downstream. |
| Execution | Claude Haiku 4.5 | 1× | 22 (implementer, reviewer, test, etc.) | Strong coding benchmarks, 1M context window. The workhorse. |
| Execution | Claude Haiku 4.5 | 1× | 1 (translation-conductor) | Leverages Anthropic-specific tool-use patterns. |
...
- Opus → Haiku 4.5 (3× → 1×)
- Claude Haiku 4.5 / Haiku 4.5 → Haiku 4.5 (1× → 0.33×)
- Haiku stays Haiku

**Result:** 3 agents on Haiku 4.5, 26 on Haiku 4.5. Good enough for learning, experimentation, and lighter workloads.
...
- name: Apply model downgrades
  run: |
    ...
    sed -i 's/Claude Opus 4\.6/Claude Haiku 4.5/g' "$file"
```
