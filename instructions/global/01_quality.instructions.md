...
|------|--------|---------------------|----------------|
| **Premium (security-only)** | Reviewer (security-mode prompt override only) | GPT-5.3-Codex -> GPT-5.3-Codex | high |
| **Execution** | Conductor, Reviewer, Implementer, Planner, Researcher, Ops, Test, IaC, GUI Tester, Translation Conductor, Translator, Translation Analyzer, Translation Validator | GPT-5.3-Codex -> GPT-5.4 mini -> GPT-5.3-Codex | low / medium / high |
...
- Premium tier is reserved for security reviews only.
- Security-mode review overrides the reviewer's array and pins `GPT-5.3-Codex` at the prompt level (`.github/prompts/support/security-review.prompt.md`).
