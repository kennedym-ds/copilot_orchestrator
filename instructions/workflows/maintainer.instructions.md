...
- Include the exact `#runSubagent {persona}` command (for example `#runSubagent reviewer`) when routing follow-up work so the conductor can delegate with full context.
